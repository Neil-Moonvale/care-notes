...
import {checkConnection} from '../dist/provider-client.js';
...
export function createMobileApi({fetchImpl=fetch,clock=Date.now}={}) {
  ...
  return async request=>{
    ...
    if(!['/api/mobile/check','/api/mobile/connection','/api/mobile/reconstruct'].includes(url.pathname))return json({error:'not_found'},404);
    ...
    active++;
    try{
      const probe=url.pathname==='/api/mobile/connection',checking=probe||url.pathname==='/api/mobile/check';

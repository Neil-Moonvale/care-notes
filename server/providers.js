...
export async function requestStructured({provider='openai',apiKey,model,fetchImpl=fetch,maxOutputTokens=6000,schema,instructions,input,name}) {
  const config=connectionConfig({provider,apiKey,model}),p=PROVIDERS[config.provider];
  ...
  let response;
  try{response=await fetchImpl(p.origin+p.path,{method:'POST',redirect:'error',headers:{Authorization:`Bearer ${apiKey}`,'Content-Type':'application/json'},signal:AbortSignal.timeout(60000),body:JSON.stringify(body)});}
  catch(error){throw Error(['TimeoutError','AbortError'].includes(error?.name)?'provider_timeout':'provider_network');}
